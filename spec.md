...
3. Gọi Claude API với prompt gồm: câu hỏi + context retrieve được + yêu cầu model tự đánh giá confidence (VD: trả JSON `{answer, confidence: high/low, sources: [...]}`)
...
```
- Cách match đơn giản nhất cho 1.5 ngày: kiểm tra keyword xuất hiện trong câu hỏi hoặc trong câu trả lời của Claude → chọn role tương ứng. Không cần ML phân loại phức tạp.
...
| LLM | Claude API (model Sonnet) | Có sẵn, tool use tốt để trả JSON có confidence |
...
| Giờ 2–4 | Build prompt Claude API (answer + confidence + sources dạng JSON) | Người B |
| Giờ 4–6 | Ghép query flow: retrieve → Claude → reply có nguồn | Người A + B |
...
| Claude API confidence không ổn định / trả lời sai | Thêm rule cứng: nếu retrieve không có chunk nào similarity đủ cao → auto route, không cần chờ Claude tự đánh giá |
